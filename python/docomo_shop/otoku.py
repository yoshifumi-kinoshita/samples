...
FROM= sys.argv[1] if len(sys.argv)>1 else 'ykinoshi@foo.example.com'
TO= sys.argv[1] if len(sys.argv)>1 else 'ykinoshi@bar.example.com'
SUBJECT='ML110 G7'
...
def send_mail( fromaddr, toaddr, msg ):
	server = smtplib.SMTP('smtp.foo.bar.example.com.')
